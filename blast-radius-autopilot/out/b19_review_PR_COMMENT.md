...
**Ambiguous references (parsed, not attributable — not safe):**
- q_adhoc_ambiguous_zip

**Patched files that WERE recomputed:** `dbt_project/models/rpt_orders_by_region.sql` → `q_dbt_rpt_orders_by_region`
...
**0 planned, 0 written (auto), 0 written (human-approved), 8 queued, 0 failed, 0 skipped**  _(dry run — nothing was written)_
...
**Queued because:** `verification_review_required+unresolved_impact` — a migration is written automatically only when static verification returned PASS.

**Approval route:** a human may approve exactly these queued mutations via `out/APPROVAL-drop-analytics-fct-orders-customer-zip.json`.
